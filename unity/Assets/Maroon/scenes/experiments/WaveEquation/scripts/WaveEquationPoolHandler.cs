...
[System.Serializable]
public class WESourceEvent : UnityEvent<WaveEquationSource>
{
}

public class WaveEquationPoolHandler : MonoBehaviour
{
    public enum Axis
    {
        XAxis,
    ...
    public WESourceEvent onSourceAdded;
    public WESourceEvent onSourceRemoved;
    ...
    private Dictionary<ulong, WaveEquationSource> _sources = new Dictionary<ulong, WaveEquationSource>();
   ...
   // private Shader _compute =  //ComputeShader(Shader.Find("Unlit/sdr WaterPW"));//Shader.Find("Unlit/sdr WaterPW");
    public static WaveEquationPoolHandler Instance
    {
        ...
        {
            if (_instance == null)
                _instance = FindObjectOfType<WaveEquationPoolHandler>();
            return _instance;
        }
    }
    ...
    private void Awake()
    {
        foreach (var waveSource in FindObjectsOfType<WaveEquationSource>())
        {
    ...
    public GameObject CreateSource(GameObject prefab, Vector3 position, float waveAmplitude, float waveLength, float waveFrequency, float wavePhase,  bool positionInWorldCoord = true)
    {
        ...
        var waveSource = obj.GetComponent<WaveEquationSource>();  
        ...
        var movement = obj.GetComponent<WaveEquation_DragHandler>();
        if (!movement) movement = obj.GetComponentInChildren<WaveEquation_DragHandler>();
        if (movement)
        {
    ...
    public void AddSource(WaveEquationSource WaveEquationSource)
    {
        SimulationController.Instance.StopSimulation();

        WaveEquationSource.SetSourceKey(_sourceIdCount);
        _sources.Add(_sourceIdCount++, WaveEquationSource);
        waterPlane.RegisterWaveSource(WaveEquationSource);

        onSourceAdded.Invoke(WaveEquationSource);
    }

    public bool ContainsSource(WaveEquationSource waveSource)
    {
        return _sources.ContainsKey(waveSource.GetSourceKey());
    }

    public void RemoveSource(WaveEquationSource source)
    {
    ...
    {
        foreach (KeyValuePair<ulong, WaveEquationSource> source in _sources)
        {
